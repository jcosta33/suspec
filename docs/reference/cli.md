...
- `suspec setup` previews, installs, checks, or removes the canonical user-level economy policy for
  explicit Codex, Claude Code, or OpenCode targets. It owns only a neutral inline block in each normal
  native instruction file. Drift or ambiguous configuration blocks mutation.
