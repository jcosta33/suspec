...
| **durable record**            | A workspace artifact kept for the repo's life and *superseded-not-deleted* — a spec of record, a decision (ADR), a saved finding. Opposite of **transitory output** (ADR-0096).                                              |
...
| **oversized-packet check**    | A size heuristic (changed-LOC + files-touched) named in ADR-0094, then *measured and deferred* — a raw band cannot be both useful and low-false-positive for code task diffs (ADR-0097). Specified-not-shipped; instead `swarm review` surfaces the diff size as **neutral info** for the reviewer to judge, no threshold asserted.                          |
...
| **transitory output**         | Short-lived workspace output — a review packet, `swarm check` output, a run log — that ages out (git history or `archive/`, a 30–90-day window) once its durable record captures what mattered. Opposite of **durable record** (ADR-0096). |
