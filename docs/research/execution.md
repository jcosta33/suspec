...
This grounding stands on REFLEXION alone. A "21× file-state degradation" figure attributed to an "InfiAgent" paper circulates in the skill-authoring literature; that arXiv id was found to resolve to an unrelated condensed-matter physics paper and is **rejected** (see [sources.md, Rejected](sources.md#rejected--do-not-cite-fabricated--misattributed--unconfirmed)). The externalisation point is instead carried by REFLEXION plus the kernel's verified context-engineering guidance — no fabricated multiplier is needed to make it.
