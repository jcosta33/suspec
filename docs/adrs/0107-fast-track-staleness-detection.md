...
([ADR-0077](./0077-corpus-cli-reconcile-only-harness.md) D8, [ADR-0056](./0056-adversarial-self-review-completion-discipline.md),
...
corpus-works#72.1 proposed a **provisional agent verdict** — but a self-issued verdict, even timed, is
...
- The review packet pins the **reviewed commit SHA + an evidence hash** (over the diff + the cited
  evidence). `corpus check` re-validates the hash on any later check; a SHA/hash mismatch flips the
...
- The solo flow: the independent reviewer agent (`corpus-reviewer`, fresh context, no verdict) prepares
...
- **Build deferred** (corpus-cli reconcile feature: the hash + the `Stale` re-validation). Reuses the
