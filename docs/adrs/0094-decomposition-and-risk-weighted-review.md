...
1. **How should a task be shaped?** The docs said "a small cleanup is still a task" but never made
   *small, single-concern, untangled, refactor-separated* the primary decomposition rule.
...
A web-verified evidence pass (swarm-hq #53; sources below, each verified June 2026 with honest
...
- **Untangling buys *cleaner* reviews, not *more* bugs caught.** A controlled experiment (n=28) found
...
- **Counter-evidence to "net-new is safe":** once size and total changes are controlled, change *type*
...
1. **Small, single-concern, untangled, refactor-separated is the primary task-decomposition rule**
   ...
   exception in [[GOOGLESMALLCL]] stands). *Level: convention.*

2. **An oversized-packet heuristic is named as a toolable signal** (`swarm check` / a step-bar):
   ...
   ([[BOSU15]]). Thresholds ship as **heuristics with provenance, never enforced law**. *Level:
   toolable (the checker is `swarm-cli`); not built here — this ADR specifies it, swarm-cli implements.*
...
3. **Review scrutiny is weighted by change-type / diffusion / churn, not greenfield-vs-brownfield**
   ...
   skipping review on a large or high-diffusion change ([[HINDLE11]]). *Level: checklist.*
...
- No `checks.yaml` rule and no contract-version bump land with this ADR: the oversized-packet flag is
  specified-not-shipped (a `swarm-cli` toolable, demand-gated), consistent with the honesty framework
