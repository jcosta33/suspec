...
Dogfooding the reference CLI surfaced the answer. `swarm check` over the swarm-hq workspace is red
with ~29 cross-spec C002 findings, because every spec naturally numbers its requirements from
`AC-001` and the kit's `swarm new spec` scaffold mints `AC-001` for each new spec — so any two
...
`checks.yaml` is the same, so the contract version and the swarm-cli drift guard are unaffected; only
...
(`swarm-cli .../checkWorkspace.ts`).

## Consequences

- `swarm check` greens against the swarm-hq dogfood workspace; the cross-spec C002 findings clear, and
  every spec (and the `swarm new spec` scaffold) can number from `AC-001` without coordination.
...
- The reference impl drops the `requirementIdToPaths` accumulation/emit in `checkWorkspace.ts`; the
  frontmatter-`id:` collision check stays. swarm-cli's `checkWorkspace` tests assert the new behavior
