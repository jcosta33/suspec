...
| Keep persona × document-type routing | Same duplication on a second axis; profile × pass replaces both legacy matrices, and a repo MUST NOT reintroduce either (§27.4). |
...
- Collapses both legacy persona matrices onto one axis (profile × pass), removing the per-cell duplication of the same mindset.
