...
again. Committed ([ADR-0060](./0060-corpus-workspace.md)) and aged-out by convention
...
1. **Gitignore the working set; commit only durable truth.** The test: *will it be re-read by future
   ...
   `corpus check` output, run logs, agent scratch. Noise never committed cannot accumulate — the
...
- **Reverses** [ADR-0060](./0060-corpus-workspace.md)'s commit-the-flow-artifacts decision — re-adopting
