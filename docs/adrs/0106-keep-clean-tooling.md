...
durable noise can only come from **tooling**. Ratified from RFC-keep-clean-tooling (corpus-works#72
items 2/5/6/8; corpus-cli#1).
...
1. **Derived index/board** — `corpus status` derives the board from live artifacts; never a
   hand-maintained list. _Toolable (already partly shipped)._
2. **`corpus clean` / gc** — **delete** the gitignored ephemeral past its window; **archive** any
...
5. **Spec-coverage drift** (corpus-cli#1) — advisory now, a check once measured 0-FP.
...
**Cost/usage visibility** (corpus-works#72.8) **splits to its own RFC** — it crosses the reconcile-only
boundary ([ADR-0077](./0077-corpus-cli-reconcile-only-harness.md): corpus-cli has no token data; the
