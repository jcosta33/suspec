...
The 10-aspect simulation audit (``.agents/audits/critical-review-aspects.md``)
...
- **#3 — adequacy advisory by default.** ``verify` §6.2` says a `RISK high|critical`
...
- **#8 — uncovered-bug seam.** A `bug-report` records "no existing obligation covers the broken behavior" as a
  gap "the fix task must reconcile" (``bug-report.md``), but the reconciliation
...
1. **An empty in-scope set does not pass by vacuity.** (``review` merge gate`) A change set
...
2. **Oracle adequacy is gating for `RISK high|critical`.** (``verify` §6.2` +
   ``review``) For a `RISK high|critical` obligation, an **inadequate oracle** — a bare
...
3. **An uncovered bug forces a spec amendment before its fix clears the gate.**
   (``bug-report.md`` + ``author``) The **fix task** a
...
- **Neutral / accompanying reconciliations (no normative change):**
  - **`SOL-M001` (#7):** ``SOL.md``'s error-table row is aligned to the **canonical**
    definition in ``errors.md`` / ``lint.md`` — *actor/object/surface
...
- **Grounded by:** the 10-aspect simulation audit (this session), aspects #3 / #4 / #8; oracle inadequacy is
  ...
  passing).
- **Changes:** the ``review`` merge-gate predicate (non-vacuity floor + adequacy-for-high-RISK);
  ``verify` §6.2` (high|critical adequacy BLOCKING); ``bug-report.md``
  + ``author`` (uncovered-bug → amendment seam). Accompanying: ``SOL.md``
  `SOL-M001` row; the hash-field contracts in ``trace.md``, ``lower.md``,
  ``structured-form.md``.
