...
That source is the new **`## Commands`** section of `AGENTS.md`. It maps the contract names to placeholders and to project commands: **required** entries (`Validation` → `{{cmdValidate}}`, `Test` → `{{cmdTest}}`, `Format` → `{{cmdFormat}}`) plus **extended** ones bound when the relevant work occurs (`Install`, `Typecheck`, `Build`, `ValidateDeps`, `Benchmark`). One table feeds both the prose references (read by the agent) and the placeholder bindings (filled by the launcher).
