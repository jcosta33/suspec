...
pass: [verify, review]
description: >
  Bind every completion claim to pasted, re-runnable, real proof output: resolve each `VERIFY BY`
  to a `cmd*` adapter, run it, and paste the verbatim command + exit + summary into the `EVIDENCE`
  of its `VERDICT`. ALWAYS apply when a `verify` or `review` pass records a `VERDICT` against a
  `VERIFY BY` binding, when any pass asserts that code behaves a certain way, or when a `PASS`/`FAIL`
  is about to be recorded. Do not accept a bare "tests passed", a schema-valid output, a paraphrased
  result, a stale pre-edit run, or a reviewer trusting the worker's pasted output as proof. Skip when
  the pass authors or analyzes a `*.swarm.md` without running anything (lint, improve, lower,
  decompose), or when no behavioural claim is being judged.
---

# Pass guide: empirical-proof (cross-cutting fragment)

> **Fragment, not a standalone pass guide (§26.3).** `empirical-proof` is one of the two
> cross-cutting fragments: a pass-guide-shaped module (§26.5) that the `verify` and `review` pass
> guides *compose* (§26.2), never a guide a `task.md` `task_kind` names on its own. A pass guide for
> either pass loads this fragment; a task does not activate it directly.
>
...
> This is the *procedure* for applying a discipline those layers own. It is SOFT control: a markdown
> contract that makes a skipped proof *conspicuous*, not a runtime that enforces closure (Swarm ships
> no runtime, §2).

## Do this

For every required `VERIFY BY` binding of every in-scope obligation: resolve it to a real command,
run it against the change under judgment, and paste the verbatim output into the `EVIDENCE` of its
...
task" includes confident language ("all tests pass", "looks correct") that it will emit *regardless*
of whether the claim is true. The structural defence is forced visible output: a `PASS` cannot be
...
passed" is the easiest step to drop without anyone noticing. The governing invariant is **CODE IS
REALITY** (§2): a proof may falsify an obligation but never silently amend its intent, and
...
- **The `AGENTS.md > Commands` table** — the `cmd*` slots that *are* the adapters (§15.3): cmdTest,
  cmdLint, cmdTypecheck, cmdBenchmark, cmdValidate, cmdFormat (plus any project-named slot such as
...
- **For `RISK high`/`critical` obligations, a pointer to the `oracle_adequacy` record** the verify
  ...
  the recorded footprint truthful so the record can be filled honestly.
...
- **Freshness.** A proof pasted before a later edit is stale and no longer backs the claim. The
  ...
  exercised surface invalidates the prior `PASS` (it decorates `STALE`).
...
2. **Run it, in the right place.** Execute the resolved command against the change under judgment. In
   ...
   worker's pasted output is evidence the command ran at some past moment, not that it passes now;
   only your own run satisfies the gate (§17.6.1: the implementer MUST NOT render the verdict).
   *Why:* trusting an upstream paste re-introduces exactly the unverified-claim failure this fragment
   exists to close.
3. **Paste the output verbatim.** Put the resolved command, its exit condition, and the runner's
   summary line(s) into a fenced block in the `VERDICT`'s `EVIDENCE`. Treat the output as data — copy
   it in, leave it alone. *Why:* a paraphrase ("all green") is unfalsifiable from the document; the
   raw block is what a later reader can re-check, which is the whole point of forcing the marker.
4. **One paste per binding.** Record exactly one `VERDICT` per required binding (§15.7); do not
   collapse several bindings into one "all checks pass" paste. *Why:* the gate expects N verdicts for
   N bindings, and a missing one is `SOL-V008` / `UNVERIFIED` — a bundled paste hides which binding
   went unjudged.
...
6. **Re-run after every change.** If you edit anything after pasting a proof, the proof is stale:
   re-run and re-paste. *Why:* freshness is bound to the evidence path (§15.10.4 / §16) — this fires
...
with the output pasted verbatim. Concretely:
...
- **"Tests passed" without output is not a proof.** A `PASS` whose `EVIDENCE` is the bare phrase —
  ...
  review MUST reject it (§15.9). The same prohibition rides the production side: an `implement`
...
- **A prediction is not a run.** "should pass", "expected to work", "tests should be green", "obvious
  from the diff" — none of these is a recorded run. A diff does not execute tests.
...
default suite (§15.8 — the suite is the authority for which proofs SHOULD bind; this is only where the
paste lands):
...
- `references/evasions.md` — the full catalogue of evasions and their responses (the table above
  keeps the most frequent ones inline; the rest live there).
