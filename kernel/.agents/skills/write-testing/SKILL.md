...
activates_for_task_kind:
  - testing
description: >-
  Run an `implement` pass whose `task_kind` is `testing`: add or strengthen tests as the
  deliverable, each behaviour-focused and proven by the flip-the-assertion oracle. ALWAYS apply
  when a `task.md` names `pass: implement` with `task_kind: testing`, or the work is to add tests,
  close a coverage gap, write a regression test, or harden a suite — including a promoted
  "Needed: test" item — even if no spec is named. Do not assert on internals, bundle unrelated
  behaviours, chase a coverage percentage, ship a test you did not flip, or edit production code to
  pass a test without an authorising obligation. Skip for implementing or fixing code itself (tests
  ride inside those deliverables), stabilising an existing flaky test, refactors, rewrites,
  migrations, or performance tuning.
---
...
> `SOL-O005` owned-path rule, the COVERAGE gate) is *delivered*, not redefined here. Where this
> guide and the spec disagree, the spec governs. It carries the **Test-Author** stance: a test is a
> specification by other means — make it fail for one reason, exercise the behaviour the caller
> cares about, and survive a refactor that preserves that behaviour.
...
net-negative — they cost maintenance and catch nothing. This guide is the discipline that keeps a
`testing` deliverable honest: behaviour-focused, one-reason-to-fail, proven to actually fire, and
robust against refactors that preserve behaviour.

This is one branch of the most-run pass of the nine (`author → lint → improve → lower → decompose
→ implement → verify → review → promote`). It is for **adding or improving test coverage as a
deliverable in its own right**. It is **not** for writing tests *as part of* building a feature or
fixing a defect (tests are already part of those deliverables, not a separate phase), nor for
stabilising a test that already fails non-deterministically — each of those is a different
discipline with its own oracle.

## Project context (the `cmd*` slots)

Resolve project commands through the consuming repo's `AGENTS.md > Commands` slots: the test
command (`cmdTest`) and the aggregate validation command (`cmdValidate`). A `testing` task often
also wants a **coverage report** and a per-test **loop or focus runner** (to run one test in
isolation, or to flip-and-rerun cheaply) — these are not part of the standard `cmd*` contract. If
`AGENTS.md` is missing, or a slot you need (including a coverage or single-test runner) is
undefined, **ask the user** which command to run before declaring any new test "passing" — never
guess a command, because a guessed command produces a false proof.

## Consumes

- **One `task.md`** — the lowered work packet for this single pass, not the surface spec or the
  IR. You read, in particular: the assigned obligations pasted verbatim (the `REQ` / `CONSTRAINT`
  / `INVARIANT` / `INTERFACE` blocks that fix what behaviour you are testing); the `write_surfaces`
  (your owned paths — here, the test files and any test-support surface, never the production code
  unless an obligation authorises it); the `verification_bindings` (the check each criterion
  demands); and the `## Scope` In/Out list.
- The driving doc the packet points at when there is one (the spec, audit, or bug report whose
  "Needed: test" item generated this work) — it tells you which behaviour the test must encode.
...
- For every test, the pasted **flip-the-assertion** transition — the test failing when its
  assertion is flipped (or the production path it exercises is commented out) and passing when
  restored — proving the test is not a tautology.
...
- **Only the assigned obligations.** The behaviour you test is fixed by the assigned obligations;
  a test for behaviour another packet owns is out of scope. Any change not traceable to an assigned
...
- **Only the declared write surfaces.** Owned paths MUST stay a subset of the union of the assigned
  obligations' `WRITES` surfaces — the owned-path rule. A path touching a file outside any assigned
  obligation's declared write surface is lint code `SOL-O005`. In particular, **do not edit
  production code to make a test easier to write** unless an assigned obligation authorises it: that
  inverts the dependency (the test exists because of the code's behaviour) and almost always lands
  outside your surfaces. Hard-to-test code is a *finding to promote*, not a licence to weaken
  production behaviour.
- **The signal of the suite.** A test you add must fail when the behaviour it guards is broken and
  pass when it is correct. A test tuned until it is green is worse than no test — it manufactures
...
- **A test you did not flip.** "It passes" is unfalsifiable from a green tick alone — the test could
  ...
  flip-and-rerun transition pasted, the test is unproven.
- **A test on internals.** Assertions on private methods, module-private state, or other internals
  break on a behaviour-preserving refactor — that is the test failing, not the code. Exercise the
  public surface.
- **A bundled test.** Multiple unrelated behaviours asserted in one case. When it fails it says
  "something broke" without saying which behaviour — split it, one reason to fail per test.
...
- **A criterion-bound test that fires for the wrong reason.** When the test is the oracle for a
  spec acceptance criterion, a green flip proves only that it fires — *not* that it fires for the
...
verbatim, the constraints and invariants to preserve, and the driving doc's "Needed: test" item if
there is one. *Why:* `decompose` already computed the work-packet boundaries; the packet — not the
surface spec or the IR — fixes which behaviour you are testing, and reading the spec instead risks
testing behaviour another packet owns.

### 2. Name the coverage gap as a behaviour, before writing any test

State precisely which module, which behaviour, and which conditions are untested or undertested —
in terms of *behaviour the caller cares about*, not lines or files. *Why:* a gap phrased as "this
file is 40% covered" produces shallow tests that chase the percentage; a gap phrased as "the
retry path does not back off on the third failure" produces a test that catches a real bug.

### 3. Confirm the owned paths

Verify your `write_surfaces` are a subset of the assigned obligations' `WRITES` surfaces — for a
`testing` task that is the test files and any test-support surface, not the production code. *Why:*
this keeps parallel `implement` packets write-disjoint (the owned-path rule; a violation is
`SOL-O005`). If making the behaviour testable would require editing production code outside your
surfaces, stop: that is a finding to promote upstream, not a surface to widen here.
...
discovered or run — it is a green tick that never executes, so it guards nothing while reading as
coverage.
...
test coupled to internals breaks on a behaviour-preserving refactor (a false failure), and a bundled
...
*Why:* `AssertionError: expected true` tells the next developer nothing; the value of a test is the
...
randomness. *Why:* a flaky test trains developers to ignore failures — the worst possible outcome,
because it disarms every other test in the suite. (A test that already fails non-deterministically
is a *different* task kind — stabilise it under its own discipline, do not author a new flake here.)

### 8. Flip the assertion to prove each test means something

After writing each test: flip its assertion (or comment out the production code path it exercises);
run that single test — it MUST fail. Restore — it MUST pass. Paste a representative sample of the
...
reason, or never run. The flip is the only evidence the test actually exercises the intended code
path and would fire if the behaviour regressed.
...
intend (the test-oracle problem the proof-strength order, §5.5, and oracle adequacy, §6.1, exist to
guard). If no fail-when-violated test can be built, that is a finding for the spec author (rebind
the criterion to `command` / `manual`), never a licence to ship a green-but-irrelevant test.
...
*Why:* the number is a diagnostic for where to look, never a target to hit; optimising the
percentage produces tests that touch lines without exercising behaviour.
...
runner's last lines in a fenced block, unmodified, treated as data, no paraphrase and no Markdown
styling. *Why:* the verbatim paste closes the bypass where "it passed" is asserted but the command
never ran; `proof_result` is the *observed* outcome — the uppercase verdict it maps to is decided
...
*Why:* a bug a test exposed is the highest-value finding the pass can produce; if it is not written
down it is lost the moment the session ends.

## Forced visible output

Compliance here is otherwise invisible — "I wrote the tests and they pass" reads identically whether
or not the tests actually exercise anything. So the hard gate is **pasted, verbatim, re-runnable**
proof, never a paraphrase and never a prediction:

- **The flip transition for every new test** — the test failing when its assertion is flipped (or
  the production path commented out) and passing when restored. This is the proof that the test is
  not a tautology; a green tick without it proves nothing.
...
no quoting, no Markdown styling, no inline annotation, treat the output as data. A test with no
pasted flip is unproven; a "tests passed" with no pasted output is not a proof.
...
- **In a testing task:** chasing a coverage percentage; refactoring production code "while you're
  here" to make testing easier without an authorising obligation (that is `SOL-O005` and a finding
  to promote); stabilising an existing flake (a different `task_kind`); fixing a production bug the
...
- ❌ Editing production code to make a test easy → that inverts the dependency and lands outside your
  surfaces (`SOL-O005`); promote the hard-to-test design as a finding instead.
...
- **Did I stay inside the owned paths?** No file outside the union of assigned `WRITES` surfaces was
  touched (no `SOL-O005`).
...
- **Deterministic?** No ordering, timing, shared-state, network, or randomness dependency that could
  flake under CI.
- **Are all promotion items resolved?** No bug the tests exposed, and no hard-to-test finding, is
  left unpromoted.
...
When the Test-Author stance carries its own self-review checks, run those too — they add checks,
...
- `references/task-template.md` — a fillable testing-task frame (coverage gap, a test-cases table
  ...
  including a coverage or single-test runner), and fill it in as you work.
