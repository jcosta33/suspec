...
`lint` consumes a `spec.swarm.md` and emits a **lint report** — an array of diagnostic records plus an overall blocking status — without changing a single character of the spec. It is the highest-leverage pass because it catches defects *before any work is committed*.
