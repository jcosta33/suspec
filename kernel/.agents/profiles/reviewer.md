...
> A **heuristic profile** is an optional cognitive stance applied to a pass (§27.1). It changes *what an agent looks for and refuses* while running the pass; it never changes *how* the pass runs (that is the pass guide, §26) and it never defines load-bearing meaning. This profile defines **no** modality, authority order, verdict value, proof type, lint code, merge-gate predicate, or any IR field — those live exclusively in the language reference (`.swarm/kernel/language/`), the typed IR, and the `review`/`verify` pass contracts, per the semantic-ownership prohibition (§26.1). Where this file names a verdict (`PASS`, `UNVERIFIED`, …), a proof type, the proof-strength order, or the merge gate, it is **citing** SOL/IR and the pass contracts, not defining them.

**This is a convenience alias, not a fourteenth persona.** `reviewer.md` carries the **Skeptic** stance (`profiles/skeptic.md`, the canonical reference profile, §27.2) narrowed to the single pass it most often parameterizes: `review` (§27.3, row 1 — `Skeptic → review / verify`). The kernel ships exactly **13** heuristic profiles; the stdlib ships a six-file subset (§20.0: `builder.md`, `skeptic.md`, `architect.md`, `researcher.md`, `reviewer.md`, `janitor.md`), in which `reviewer.md` is the named alias and the substance lives in `skeptic.md`. When a task names `review[profile: skeptic]` (§9.3) and when it names `profile: reviewer`, the stance is identical; this file exists so that `profile: reviewer` resolves to a present, conformant carrier for the `review` pass without minting a new mindset. The broader Skeptic scope (also `verify`, and the `fix` task kind for root-causing, ADR 0006 recast) lives in `skeptic.md`; load that when the pass is not `review`.
...
The stance demands this evidence *before* it accepts a claim. (What counts as a proof, the closed proof taxonomy, and the verdict vocabulary are defined in the `review`/`verify` pass contracts — cited here, not redefined.)
...
- **Findings cite file and line.** Every finding names a specific surface and a specific issue; a vague concern is not a finding. (Severity, fix sketch, and verdict semantics are owned by the `review` pass contract, not by this stance.)
...
The proof-strength order, the seven-value verdict model, and the merge-gate predicate that make these dispositions binding live in the `review`/`verify` pass contracts; the reviewer refuses *under* them, never *over* them.
