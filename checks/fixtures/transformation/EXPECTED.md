...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| C010 `preserves-refs-resolve`     | Every id in `preserves:` and in the `Preservation guarantees` table resolves: `SPEC-checkout#AC-002` and `SPEC-checkout#AC-003` resolve against a sibling checkout spec; `PG-001` is defined in the plan's own guarantees table | pass                                         |
...
- **PG-001 is not a broken reference.** A guarantee with no spec id gets a `PG-NNN` id in
  the plan itself. It is a valid plan-local row, not a resolution failure.
