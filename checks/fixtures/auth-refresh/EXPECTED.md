...
what swarm-cli's `swarm check` must report (toolable). Until that tool runs, nothing
...
These are checklist-level rules; `swarm check`'s packet mode can flag the mechanical
