...
Like every Swarm pass, `promote` has **no runtime**: it is a contract a human, an agent following a pass guide, or a future tool performs. Swarm ships the *files and the discipline* a retrieval/promotion tool would build against — not a retrieval engine, validation scorer, or eviction manager. Nothing here is shipped code.
...
|---|---|
| Pass guide (a sibling skill / pass-guide file under `starter-kit/.agents/`) | The actual procedure / steps |
