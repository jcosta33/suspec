...
| **Framework / package version** | The starter kit — templates, pass guides, profiles, the flow-graph, and skills that ship together | a **producer release tag** on the `swarm` repo. An adopted project keeps **no** copy (ADR-0050) | Ordinary, fast semver; may move many times between language bumps |

(The block-type, modal, and lint-layer counts above are Swarm's fixed vocabulary — 7 block types, 5 modals, 5 lint layers S/P/M/V/O. This page reproduces those counts as labels for the language axis; the vocabulary itself is defined in `./SOL.md` (blocks and modals) and `./errors.md` (lint layers).)
