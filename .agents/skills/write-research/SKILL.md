...
type: agent-guide
description: >-
  Write a research note: survey the options and evidence behind ONE
  decision-informing question, and commit to no decision. ALWAYS apply when
  asked for research, an options / library / API comparison, an evidence
  survey, or a recommendation feeding a later decision. Never present opinion
  as a finding, cite a blog without its primary source, fabricate or leave a
  claim unmarked when you could not verify it, settle an open question by
  asserting a choice, or write requirements. Skip when writing a spec, an
  audit of present state, or a defect diagnosis — research is their upstream
...
made later, when someone lifts the findings into a spec or an ADR. The format is the kit
template `starter-kit/advanced/research.md` (Question · Findings · Open questions ·
Recommendation); do not reinvent its sections. In this repo a research note lives beside the
work it informs (the repositioning inputs sit under `.agents/plans/`); in an adopted workspace
it lives in `specs/<feature>/`.

The discipline is evidentiary: cite or omit. A claim that cannot survive a citation falls out of
the document. These rules are conventions backed by review — nothing in this repo enforces them.

## The stance

Investigate one question in depth against primary sources; keep observation distinct from claim,
keep your confidence honest, and refuse to let the inquiry harden into a decision — the
Researcher stance (`../persona-researcher/SKILL.md`). For a breadth survey instead — what
prevails across many products, patterns, or users — load `../persona-surveyor/SKILL.md`; it
carries the extra rules breadth needs.
...
first option found is how a survey silently becomes an advocacy piece for whatever you read
first.
...
next reader sees it. (In this repo the bar is higher still: a source enters
`docs/research/sources.md` only after web-verifying its venue and finding.)
...
- Requirements in any form: no AC items, no SOL blocks
  (`docs/reference/structured-requirements.md`) — those belong in the spec someone writes
  _from_ this note.
...
Paste the visibility table into your task file — any ❌ means the note is not deliverable yet:

| R-NNN | Evidence non-empty? | Confidence set? | Verified, or `[unconfirmed]`? |
| ----- | ------------------- | --------------- | ----------------------------- |
...
- [ ] The recommendation names the findings it rests on — or names the open question blocking
      one.

## Bundled resources

- `references/task-template.md` — a fillable session frame: metadata, constraints, plan,
  progress checklist, the deliverable structure inlined, the visibility table, and a
  self-review hard gate. Instantiate it into your task file, fill it as you work, and copy the
