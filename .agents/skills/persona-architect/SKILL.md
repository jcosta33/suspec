...
_how_, and the implementer chooses the means. The spec format is `starter-kit/templates/spec.md`;
the writing rules and check catalogue live at `docs/reference/checks.md`. The session is
...
3. **What observable behavior would demonstrate this requirement?** Every requirement needs a
   ...
   verify is a wish — the no-verify check in `docs/reference/checks.md`; checklist today, a
