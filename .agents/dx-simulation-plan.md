...
| **P6** | `UPGRADING.md` + an overlay survival seam | M · low · P1 | A preserve-list + safe-replace sequence + post-upgrade re-grade checklist; relocate `overlays/` to a sibling `.swarm/overlays/` (or mark it project-owned-preserve with a stash/restore step) so a payload swap can't clobber it. |
